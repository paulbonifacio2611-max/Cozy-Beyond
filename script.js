...
document.querySelectorAll('.star-rating input').forEach((input) => input.addEventListener('change', () => { const rating = Number(input.value); document.querySelectorAll('.star-rating label').forEach((label) => { const labelRating = Number(label.htmlFor.replace('rating-', '')); label.textContent = labelRating <= rating ? '★' : '☆'; }); }));
reviewForm?.addEventListener('reset', () => { document.querySelectorAll('.star-rating label').forEach((label) => { label.textContent = '☆'; }); });
reviewForm?.addEventListener('submit', (event) => { event.preventDefault(); const formData = new FormData(reviewForm); const message = document.getElementById('review-message'); const name = String(formData.get('name') || '').trim(); const rating = Number(formData.get('rating')); const review = String(formData.get('review') || '').trim(); if (!name || !rating || !review) { message.textContent = 'Please add your name, rating, and review.'; message.className = 'form-message error'; return; } const card = document.createElement('article'); card.className = 'review-card'; const stars = document.createElement('div'); stars.className = 'review-stars'; stars.setAttribute('aria-label', `${rating} out of 5 stars`); stars.textContent = `${'★'.repeat(rating)}${'☆'.repeat(5 - rating)}`; const quote = document.createElement('blockquote'); quote.textContent = review; const author = document.createElement('p'); author.className = 'review-author'; author.textContent = name; const date = document.createElement('time'); date.className = 'review-date'; date.dateTime = new Date().toISOString().slice(0, 10); date.textContent = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }); card.append(stars, quote, author, date); reviewsList?.querySelector('.empty-reviews')?.remove(); reviewsList?.prepend(card); message.textContent = 'Thank you for sharing your experience.'; message.className = 'form-message success'; reviewForm.reset(); });
